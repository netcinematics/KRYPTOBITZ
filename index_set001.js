...
let {BITZSET, TOTAL_CARDZ, SET_TXT, METANET, RARITYNET, IDENTITYNET, OS_META_MODEL, width, height} = require('./config1.js')
...
if(COMMANDZ[0]==='2'){ //IPFS MODE-.
    ...
    let rawdata = fs.readFileSync(`${rootPATH}/${SET_TXT}/json/_metadataMAIN.json`);
    ...
    data.forEach((item) => {  //Update metadata to OpenSea data structure
        ...
        if(!fs.existsSync(`./${SET_TXT}/json/${publishDateStr}`)){ fs.mkdirSync(`./${SET_TXT}/json/${publishDateStr}`); }
        fs.writeFileSync(`${rootPATH}/${SET_TXT}/json/${publishDateStr}/${item.cardNum}.json`,
            JSON.stringify(item, null, 2)
        );
    });
    //FILE: WRITE - OUT IPFS REPLACEMENT in METADATA MAIN.
    if(!fs.existsSync(`./${SET_TXT}/json`)){ fs.mkdirSync(`./${SET_TXT}/json`); }
    fs.writeFileSync(  
      `${rootPATH}/${SET_TXT}/json/_metadataMAIN.json`, JSON.stringify(data, null, 2)
    ...
    // DATE_STAMP_TAG += `_${new Date().toTimeString().split(':')[0]}_${new Date().toTimeString().split(':')[1]}`        
    if(!fs.existsSync(`./${SET_TXT}/json/bak/${DATE_STAMP_TAG}`)){ fs.mkdirSync(`./${SET_TXT}/json/bak/${DATE_STAMP_TAG}`); }
    fs.writeFileSync(
        `./${SET_TXT}/json/bak/${DATE_STAMP_TAG}/_metadataMAIN.json`, JSON.stringify(data, null, 2)
    );
    return; //END SCRIPT-.
}//END IPFS MODE-.
...
\************************************************************/
   ...
   .then( (imageSet) => { // .all([ logo, cp, tm, sky1,bg1,hero1 ])
    ...
    .then(function(data){ // console.log('LOADED IMGS',data)
        ...
        if(!fs.existsSync(`./${SET_TXT}/images/IMGZ`)){ fs.mkdirSync(`./${SET_TXT}/images/IMGZ`); }
        fs.writeFileSync(`./${SET_TXT}/images/IMGZ/${_currentCardNum}.png`,canvas.toBuffer("image/png"))
        //BAK IMGZ, to ignored bak folder. dated on folder. Has the stamp on the card.
        // let DATE_STAMP_TAG = `${new Date().toISOString().split("T")[0]}`
        // DATE_STAMP_TAG += `_${new Date().toTimeString().split(':')[0]}_${new Date().toTimeString().split(':')[1]}`        
        if(!fs.existsSync(`./${SET_TXT}/images/bak/${DATE_STAMP_TAG}`)){ fs.mkdirSync(`./${SET_TXT}/images/bak/${DATE_STAMP_TAG}`); }
        fs.writeFileSync(`./${SET_TXT}/images/bak/${DATE_STAMP_TAG}/${_currentCardNum}.png`,canvas.toBuffer("image/png"))
        ...
        function finishMETABITZ(){ //OPTIMIZATION: SAVE ONLY ONCE- at the end-.
            if(_currentCardNum===TOTAL_CARDZ){ //FINAL-CARD-. Finish METABITZ-.
                ...
                //FILE WRITE: METADATA MAIN - original run. No need to BAK. Still needs IPFS-.
                 fs.writeFileSync(`./${SET_TXT}/json/_metadataMAIN.json`, JSON.stringify(METABITZOS));
                writeMETANETtoFILE();
...
function startMETABITZ(){ 
    try {
        ...
        let rawdata = fs.readFileSync(`${rootPATH}/${SET_TXT}/json/KBZ_METANET.json`)
        let data = JSON.parse(rawdata);
...
function writeMETANETtoFILE(){ //write to master-meta-net-.
    ...
    var stream = fs.createWriteStream(`./${SET_TXT}/json/KBZ_METANET.json`, {flags:'w'}); //a append, w write-.
    // var stream = fs.createWriteStream(`./${SET_TXT}/KBZ_METANET_${new Date().toISOString().split(":")[0]}.json`, {flags:'a'});
    ...
    // let DATE_STAMP_TAG = `${new Date().toISOString().split("T")[0]}`
    // DATE_STAMP_TAG += `_${new Date().toTimeString().split(':')[0]}_${new Date().toTimeString().split(':')[1]}`        
    // if(!fs.existsSync(`./${SET_TXT}/bak`)){ fs.mkdirSync(`./${SET_TXT}/bak/${DATE_STAMP_TAG}`); }
    // fs.writeFileSync(
    //     `./${SET_TXT}/bak/${DATE_STAMP_TAG}/KBZ_METANET.json`, JSON.stringify(METABITZ)
    // );
